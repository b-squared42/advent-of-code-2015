...
class Box {
    ...
    sortedSides() {
        return [this.length, this.width, this.height].sort((a,b) => {return a - b});
    }
}
...
function one() {
    ...
    let wrappingPaper = 0;
    boxes.forEach((box) => {
        wrappingPaper += (2 * box.length * box.width) + (2 * box.width * box.height) + (2 * box.height * box.length) + box.smallestWall();
    });
